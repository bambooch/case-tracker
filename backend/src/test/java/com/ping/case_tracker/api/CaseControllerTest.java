...
import com.ping.case_tracker.api.controller.CaseController;
...
class CaseControllerTest {
    ...
    @Test
    void getCaseByIdReturnsCaseDetail() throws Exception {
        this.mockMvc.perform(get("/api/cases/1"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(1))
            .andExpect(jsonPath("$.title").value("Missing documents"))
            .andExpect(jsonPath("$.status").value("OPEN"))
            .andExpect(jsonPath("$.attentionLevel").value("IMMEDIATE"))
